...
snp.stat <- fqtl.stat %>%
    select(ensg, factor, snp, snp.theta, snp.sd)
...
take.twas <- function(gg) {
    ...
    qtl.info <-
        data.frame(snp = snp.stat[gg, 'snp'] %>% .split(),
                   qtl.beta = snp.stat[gg, 'snp.theta'] %>% .split() %>% as.numeric(),
                   qtl.se = snp.stat[gg, 'snp.sd'] %>% .split() %>% as.numeric()) %>%
                       separate(snp, c('chr', 'snp.loc', 'qtl.a1', 'qtl.a2'), sep = ':') %>%
