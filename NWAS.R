...
match.qtl.gwas <- function(plink, gwas.stat, qtl.info, tss, cis.dist) {

    x.bim <- plink$BIM %>%
        dplyr::select(-rs, -missing) %>%
            dplyr::mutate(x.pos = 1:n())

    gwas.sub <- gwas.stat %>% filter(snp.loc >= (tss - cis.dist),
                                     snp.loc <= (tss + cis.dist))

    ret <- x.bim %>%
        left_join(qtl.info) %>%
            mutate(qtl.a2 = if_else(is.na(qtl.a2), plink.a1, qtl.a2),
                   qtl.a1 = if_else(is.na(qtl.a1), plink.a2, qtl.a1))
...
func.NWAS <- function(.qtl.z, .gwas.z, vt, D) {
    .num <- sum(.qtl.z * .gwas.z)
    .eta.QTL <- sweep(sweep(vt, 2, .qtl.z, `*`), 1, D, `*`)
